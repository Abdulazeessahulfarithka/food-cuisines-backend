import dinnerRecipe from "../model/dinnerRecipe.js";
...
export const createRecipe = async (req, res) => {
  try {
    ...
    const recipe = new dinnerRecipe({ name, photo,description,instructions,ingredients,photo1,benefits});
    await recipe.save();
...
export const getAllRecipes = async (req, res) => {
  try {
    const recipes = await dinnerRecipe.find({});
    res.status(200).send({
      success: true,
