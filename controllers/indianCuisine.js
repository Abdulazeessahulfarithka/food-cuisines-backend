...
export const createRecipe = async (req, res) => {
  try {
    const { name, photo, description, ingredients, photo1, benefits } = req.body;
    ...
    const recipe = new indianCuisine({ name, photo, description, ingredients, photo1, benefits });
    await recipe.save();
...
export const getAllRecipes = async (req, res) => {
  try {
    const recipes = await indianCuisine.find({});
    res.status(200).send({
      success: true,
